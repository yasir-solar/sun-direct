...
export default function TestimonialsPage() {
  return <>
    ...
    <section className="testimonials-video-section section"><div className="container"><div className="testimonials-heading centered"><h2>Hear from solar homeowners.</h2><p>Watch customer-supplied videos from the field.</p></div><div className="testimonials-video-grid">{customerVideos.map((src, index) => <article key={src}><video controls playsInline preload="metadata" aria-label={`Customer testimonial video ${index + 1}`}><source src={src} type="video/mp4"/>Your browser does not support embedded video.</video></article>)}</div></div></section>
  </>;
}
